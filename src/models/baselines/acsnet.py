...
from ..common.official_backbones import OfficialResNet34Encoder
...
class ACSNet(nn.Module):
    """ACSNet baseline using the paper's ResNet-34 encoder and LCA/GCM/ASM decoder."""
    ...
    def __init__(
        ...
        if backbone_impl.lower() in {"official", "official_backbone"}:
            self.encoder = OfficialResNet34Encoder(in_channels=in_channels, channels=channels, pretrained=backbone_pretrained, checkpoint=backbone_checkpoint, checkpoint_url=backbone_checkpoint_url)
        else:
    ...
    def forward(self, x: torch.Tensor):
        ...
        y4 = resize_to(self.pred4(resize_to(d3_seed, x3)), x)
        ...
        if self.faithful_output:
            return {"main": y0, "aux": [y4, y3, y2, y1]}
        return y0
