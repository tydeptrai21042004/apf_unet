...
class ResUNetPlusPlus(nn.Module):
    ...
    def __init__(
        ...
        **_: object,
        ...
        self.aspp_bridge = ASPP(c3, c4, rates=tuple(int(r) for r in aspp_rates), norm=norm, act=act)
        # Backward-compatible alias used by older scripts; aspp_bridge is the paper-contract name.
        self.bridge = self.aspp_bridge
        self.bridge_se = SqueezeExcitation(c4)

        self.dec4 = ResUNetPPDecoderBlock(c4, c3, c3, norm=norm, act=act)
        # Backward-compatible alias used by older code; dec4 is the paper-contract name.
        self.dec3 = self.dec4
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        b = self.bridge_se(self.aspp_bridge(s3))
        d3 = self.dec4(b, s3)
        d2 = self.dec2(d3, s2)
        d1 = self.dec1(d2, s1)
        d0 = self.dec0(d1, s0)
