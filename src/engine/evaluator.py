    ...
    def evaluate(
        ...
        *,
        ...
        for batch in dataloader:
            ...
            # Optional model-level diagnostics (e.g. CHD complementarity and
            # branch-energy ratios). These are reported beside segmentation
            # metrics without affecting the evaluation loss.
            if hasattr(model, "diagnostics"):
                diagnostic_values = model.diagnostics()
                for key, value in diagnostic_values.items():
                    if torch.is_tensor(value):
                        value = float(value.detach().mean().item())
                    totals[str(key)] += float(value) * batch_size
            samples += batch_size
