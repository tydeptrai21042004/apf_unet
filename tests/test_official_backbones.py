...
def test_official_res2net_encoder_outputs_requested_pyramid_channels():
    model = OfficialRes2NetEncoder(channels=(16, 32, 64, 128, 256), pretrained=False).eval()
...
def test_official_hardnet_encoder_outputs_requested_channels():
    model = OfficialHarDNetEncoder(channels=(16, 32, 64, 128, 256), pretrained=False).eval()
    x = torch.randn(1, 3, 32, 32)
    with torch.no_grad():
        feats = model(x)
    assert [t.shape[1] for t in feats] == [16, 32, 64, 128, 256]
