...
FAIR_MODELS="unet,attention_unet,unetpp,resunetpp,pranet,acsnet,hardnet_mseg,polyp_pvt,caranet,cfanet,hsnet,csca_unet,proposal_fourier_unet,proposal_urf_unet"
...
case "${1:-help}" in
 ...
 ablation) "$PYTHON_BIN" scripts/run_fourier_ablation.py --dataset "$DATASET" --data-root "$DATA_ROOT" --device "$DEVICE" --seed "$SEED" ;;
 urf-ablation) "$PYTHON_BIN" scripts/run_urf_ablation.py --dataset "$DATASET" --data-root "$DATA_ROOT" --device "$DEVICE" --seed "$SEED" ;;
 test) "$PYTHON_BIN" -m pytest -q ;;
 *) echo "Usage: bash run.sh {install|prepare|fair|faithful|ablation|urf-ablation|test}" ;;
esac
